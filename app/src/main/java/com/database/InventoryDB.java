...
public class InventoryDB extends GenericDao implements InventoryDao{
	...
	public Item insert(int inventoryLineItem_id , Item item) {
		...
		cv.put(Item.COL_STOCK, item.getStock());
		cv.put(Item.COL_INVENTORYLINEITEM_ID, inventoryLineItem_id);
		cv.put(Item.COL_DESCRIPTION_ID, item.getItemDescription().getId());

		return new Item((int)super.insert(Item.DATABASE_TABLE, cv) , item.getItemDescription() ,item.getCost() , item.getStock());
	}
	...
	public ArrayList<Item> findAll() {
		String[] columns = new String[]{GenericDao.KEY_ID ,Item.COL_DESCRIPTION_ID , Item.COL_STOCK , Item.COL_COST};
		...
		if(cursor != null){
			if(cursor.moveToFirst()){
				...
				for(int i = 0 ; i < count ; i++){
					...
					itds.add(new Item(cursor.getInt(_id), des , cursor.getFloat(_cost) , cursor.getString(_Stock)));
					cursor.moveToNext();
				}
				_db.close();
			}
	...
	public Item findByID(int id) {
		...
		if(cursor != null){
			...
			item = new Item(id, itemDescription , cursor.getFloat(_cost) , cursor.getString(_Stock));
		}
		return item;
	}
	...
	/**
	 * @see com.database.InventoryDao#findByStock(java.lang.String)
	 */
	@Override
	public Item findByStock(String id) {
