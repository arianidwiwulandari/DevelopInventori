			...
			public void onClick(View v) {
				...
				try {
					...
					if (itemDes == null)
						...
						alert.setTitle("Product STOCK number");
						alert.setMessage("Insert the STOCK of the product");
						...
						alert.setPositiveButton("OK",
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
										...
										if(!stock.equals("")){
											Item item = new Item(-1, inventoryController.getItemDescriptionByBarcode(getApplicationContext(), barcode), stock);
											items.add(item);

											arrayAdapter.notifyDataSetChanged();
										}
										else {
											Toast.makeText(getApplicationContext(), "Please fill the stock number", Toast.LENGTH_LONG).show();
										}
									}
								});
	...
	public void onActivityResult(int requestCode, int resultCode, Intent intent) {
		if(requestCode==1){
			if(resultCode==this.RESULT_OK){
				...
				String stock = contents;

				Item item = new Item(-1, inventoryController.getItemDescriptionByBarcode(getApplicationContext(), txtBarcode.getText().toString()), stock);
				items.add(item);
